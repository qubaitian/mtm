...
(defclass session-manager ()
  ((named-sessions
   ...
   (last-input-attachment
    ;; Store the Attachment that last sent Session input.
    :initform nil
    :accessor manager-last-input-attachment))
...
(defclass managed-session ()
  ((name
   ...
   ;; Track the Attachment that controls the PTY terminal size.
   (size-owner
    :initform nil
    :accessor managed-session-size-owner)
   (reader-thread
    :initform nil
  ...
  (let* ((manager (or (get-session-manager-value)
    ...
    (with-lock-held ((session-lock session))
      ...
      (let* ((start-screen (get-terminal-copy (managed-terminal session)))
        ...
        (unless (managed-session-size-owner session)
          (setf (managed-session-size-owner session) attachment))
        attachment))))

;; Return true when ATTACHMENT controls the PTY terminal size.
(defun attachment-size-owner-p (attachment)
  "Return true when ATTACHMENT controls the PTY terminal size."
  (check-type attachment attachment)
  (let ((session (attachment-session attachment)))
    (with-lock-held ((session-lock session))
      (eq attachment (managed-session-size-owner session)))))
...
(defun del-attachment-under-lock (attachment)
  ...
  (let ((session (attachment-session attachment))
    ...
    ;; Pass terminal size control to the next Attachment.
    (when (eq attachment (managed-session-size-owner session))
      (setf (managed-session-size-owner session)
...
(defun del-attachment-value (attachment)
  "Detach ATTACHMENT without closing its Session."
  (check-type attachment attachment)
  (let ((session (attachment-session attachment)))
    (with-lock-held ((session-lock session))
  ...
  (del-last-input-attachment attachment)
  t)

(defun set-last-input-attachment (attachment)
  "Remember ATTACHMENT as the last input source."
  (check-type attachment attachment)
  (let ((manager (session-manager (attachment-session attachment))))
    (with-lock-held ((manager-lock manager))
      (setf (manager-last-input-attachment manager) attachment)))
  attachment)

(defun get-last-input-attachment ()
  "Return the attached last-input Attachment, or NIL."
  (let ((manager (get-session-manager-value)))
    (when manager
      (let ((attachment
              (with-lock-held ((manager-lock manager))
                (manager-last-input-attachment manager))))
        (when (and attachment (attachment-attached-p attachment))
          attachment)))))

(defun del-last-input-attachment (attachment)
  "Clear last-input when it still names ATTACHMENT."
  (let ((manager (session-manager (attachment-session attachment))))
    (with-lock-held ((manager-lock manager))
      (when (eq attachment (manager-last-input-attachment manager))
        (setf (manager-last-input-attachment manager) nil))))
  t)

;; Detach the Attachment that last sent input.
(defun del-attachment ()
  "Detach the Attachment that last sent input."
  (let ((attachment (get-last-input-attachment)))
    (unless attachment
      (error "No Attachment has sent input."))
    (let ((name (session-name (attachment-session attachment))))
      (del-attachment-value attachment)
      name)))
...
;; Update SESSION's retained projection and broadcast raw output.
      ...
      (let ((accepted-p
              (handler-case
                  (with-lock-held ((session-write-lock session))
                    ...
                    t)
                (error () nil))))
        (when (and accepted-p (plusp (length input)))
          (set-last-input-attachment attachment))
        accepted-p))))
