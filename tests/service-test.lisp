...
(deftest status-bar-renders-and-selects-services ()
  (let ((collapsed
        ...
        (frontend
          (mtm.frontend::new-session-frontend
           ...
           :rows 6
           :columns 40
           :sessions '(("shell" . :running))
           :services '(("web" . :running)))))
    (setf (mtm.frontend::session-frontend-expanded-p frontend) t)
    (check (and (search "1 sessions, 1 services" collapsed)
                (not (search "web" collapsed)))
           "The collapsed bar does not summarize Services.")
    (check (search "service web" expanded)
           "The expanded bar does not render Service rows.")
    (check (mtm.frontend::set-status-bar-mouse-event frontend 0 1 4 t)
           "A Service row does not accept a click.")
    (check (string= "web"
                    (mtm.frontend::session-frontend-requested-service frontend))
           "The Service row does not request its Service.")))
